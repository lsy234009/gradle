...
class DefaultWorkerExecutorTest extends Specification {
    ...
    def "can add to classpath on executor"() {
        ...
        then:
        1 * classLoaderStructureProvider.getWorkerProcessClassLoaderStructure(_) >> { args -> new HierarchicalClassLoaderStructure(new VisitableURLClassLoader.Spec("test", args[0].collect { it.toURI().toURL() }))}
